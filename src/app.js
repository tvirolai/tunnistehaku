...
const instructions = ["MARC 21", "MARC 21 Full (LOC)", "MARC 21 sovellusohje (RDA)", "MARC 21 sovellusohje (ISBD)"];

function TextField(props) {
  return (
    <input type="text" className="form-control" placeholder={props.name} value={props.arvo} onChange={props.changeFunction} autoFocus />
  );
}
...
class Form extends React.Component {
  ...
  sendForm(event) {
    ...
    //alert(JSON.stringify(this.state)); DEBUG
  }
  ...
  render() {
    let selection = (this.state.ohjeet) ? <DbSelect title="Haku ohjeista" content={instructions} changeFunction={this.changeSelection} /> : <DbSelect title="Haku tietokannasta" content={databases} changeFunction={this.changeSelection} />;
    let textPlaceHolder = (this.state.ohjeet) ? "Syötä MARC 21 kenttäkoodi" : "Syötä tietuenumero(t)";
    let textFormatting = (this.state.ohjeet) ? "glyphicon glyphicon-console form-control-feedback" : "glyphicon glyphicon-book form-control-feedback";
    return (
